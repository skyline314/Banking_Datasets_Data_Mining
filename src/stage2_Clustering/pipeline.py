...
def run(input_path: Path, output_dir: Path) -> bool:
    ...
    try:
        ...
        df_cluster, X_scaled, X_sample, scaler = load_and_prepare(input_path)

        # -- STEP 2 / 4 : EVALUATE OPTIMAL K ----------------------------------
        log.info("-- STEP 2 / 4 : EVALUATE OPTIMAL K ------------------------")
        optimal_k = evaluate_optimal_k(X_scaled, X_sample, output_dir)

        # -- STEP 3 / 4 : CLUSTER ---------------------------------------------
        log.info("-- STEP 3 / 4 : APPLY CLUSTERING --------------------------")
        df_cluster = apply_kmeans(df_cluster, X_scaled, optimal_k)
        generate_dendrogram(X_scaled, output_dir)
        detect_outliers_dbscan(X_sample)

        # -- STEP 4 / 4 : PROFILE & EXPORT ------------------------------------
        log.info("-- STEP 4 / 4 : PROFILE & EXPORT --------------------------")
        build_and_export_profiles(df_cluster, scaler, output_dir)
